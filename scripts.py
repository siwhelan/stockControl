...
def calculate_recipe_costs(recipe, ingredients_collection):
    ...
    for ingredient, quantity in recipe["ingredients"].items():
        ...
        ingredient_info = ingredients_collection.find_one({"ingredient": ingredient})
        # Calculate the cost of the ingredient by multiplying the quantity needed by the price per pack
        ingredient_cost = quantity * ingredient_info["price_per_pack"] / ingredient_info["pack_size"]
...
if __name__ == "__main__":
    ...
    for recipe in recipes:
        ...
        recipes_collection.update_one({"_id": recipe["_id"]}, {"$set": {"recipe_cost": recipe["recipe_cost"]}})
